...
## CANopen SDO 명령 레퍼런스 (범용)

### 모터 제어 (!G 직접 명령)

Node ID = N일 때: SDO Request COB-ID = `0x600 + N`, SDO Response COB-ID = `0x580 + N`

```bash
# CH1 전진 (value=200 → MXRPM의 20%)
cansend can0 60N#23002001 00C80000

# CH2 전진
cansend can0 60N#23002002 00C80000

# CH1 정지
cansend can0 60N#23002001 00000000

# CH2 정지
cansend can0 60N#23002002 00000000
```
...
```bash
# 에러 레지스터 (0x1001:00)
cansend can0 60N#40 01100000000000

# MMOD 읽기 ch1 (0x3027:01)
cansend can0 60N#40 27300100000000

# 전류 읽기 ch1 (?A 1 = 0x2100:01)
cansend can0 60N#40 00210100000000

# 폴트 플래그 (0x2106:04)
cansend can0 60N#40 06210400000000

# FID 장치 식별 (0x1008:00)
cansend can0 60N#40 08100000000000
```

### 시리얼 진단 명령

```bash
# Python으로 Roboteq 시리얼 연결
python3 -c "
import serial, time
ser = serial.Serial('/dev/ttyACMx', 115200, timeout=1)
ser.write(b'\r\r'); time.sleep(0.2); ser.flushInput()
for cmd in ['?FID', '?FF', '~MMOD 1', '~MMOD 2', '~MXRPM 1', '~EPPR 1', '~CNOD', '~CHB', '~SCRO']:
    ser.write((cmd + '\r').encode())
    time.sleep(0.3)
    print(f'{cmd}: {ser.read(200).decode(errors=\"replace\").strip()}')
ser.close()
"
```
...
| 방식 | CAN 주소 | 스크립트 필요 | 용도 |
|------|---------|-------------|------|
| **직접 !G (SDO)** | `0x2000:ch` | 불필요 | 단순 속도/PWM 제어 |
| **User Variable (TPDO)** | `0x2005:sub` | **필요 (SCRO>0)** | 스크립트 기반 고급 제어 (호밍, 상태머신) |
